...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const CACHE_KEY = `most_favorite_page_${page}_v1`;

  // If Redis is not configured, passthrough mode (no caching)
  if (!redis) {
    try {
      const apiUrl = `${API_URL}/api/v2/hianime/most-favorite?page=${page}`;
      ...
      if (!resp.ok) {
        ...
        return new Response(
          JSON.stringify({ 
            success: false, 
            error: `Failed to fetch most favorite data: ${resp.status} ${resp.statusText}` 
          }),
          { 
            status: resp.status,
            headers: { 'Content-Type': 'application/json' }
