...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const CACHE_KEY = `added_page_${page}_v1`;

  // If Redis is not configured, passthrough mode (no caching)
  if (!redis) {
    try {
      const apiUrl = `${API_URL}/api/v2/hianime/recently-added?page=${page}`;
      ...
      if (!resp.ok) {
        ...
        return new Response(
          JSON.stringify({ 
            success: false, 
            error: `Failed to fetch completed data: ${resp.status} ${resp.statusText}` 
          }),
          { 
            status: resp.status,
            headers: { 'Content-Type': 'application/json' }
