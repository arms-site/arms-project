...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const CACHE_KEY = `updated_page_${page}_v1`;

  // If Redis is not configured, passthrough mode (no caching)
  if (!redis) {
    try {
      const apiUrl = `${API_URL}/api/v2/hianime/recently-updated?page=${page}`;
      ...
      if (!resp.ok) {
        ...
        return new Response(
          JSON.stringify({ 
            success: false, 
            error: `Failed to fetch top airing data: ${resp.status} ${resp.statusText}` 
          }),
          { 
            status: resp.status,
            headers: { 'Content-Type': 'application/json' }
